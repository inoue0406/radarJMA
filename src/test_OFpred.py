...
def OF_predictor(data_torch):
    # data : tensor with dimension of [tsize,width,height]
    # define xy grid
    ...
    flow_all = np.zeros((tsize,xsize,ysize,2))
    ofsize = 6
    for i in range((tsize-1-ofsize),(tsize-1)):
        #print('calc OF between ',i,' and ',i+1)
        flow_all[i-5,:,:] = cv2.calcOpticalFlowFarneback(data[i,:,:],data[i+1,:,:],None,
    ...
    VY = medflow[:,:,1]
    # prep interp func
    R = data[tsize-1,:,:] # use last picture as initial condition
    itp_rain = interpolate.RectBivariateSpline(xgrd, ygrd, R)
    Rp = data[tsize-2,:,:] # previous step for change ratio calculation
    ...
    return(torch.from_numpy(data_nxt))
